...
const RegisterForm = () => {
    ...
    try {
        ...
        if (contentType && contentType.includes("text/html")) {
            // Handle HTML error page (usually an error page returned by the server)
            console.error("Error page returned:", responseText);
            toast.error("An error occurred on the server.");
            return;
        }

        if (!response.ok) {
            const errorJson = JSON.parse(responseText); // We can safely parse if it's not HTML
