...
passport.deserializeUser(async (IdleDeadline, next) => {
  try {
    ...
    next(new ApiError(500, `Error while deserializing user: ${err}`), null);
  }
});
        ...
        if (user) {
          if (user.signInType !== "Google") {
            ...
            return next(
              new ApiError(
                400,
                `You have already registered using ${signInType}. Please use ${signInType} as the signin option.`
              ),
              null
            );
          } else {
